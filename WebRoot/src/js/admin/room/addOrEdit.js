...
layui.use(['layedit','form', 'element','laydate'], function(){
	...
	var layeditInstance = layedit.build('room_detail'); //建立编辑器
	/****************** 函数实现代码区域 start ******************/
	/**
	 * 
	 */
	...
	function renderBuildSelect(buildings){
		if (buildings && buildings.length > 0) {
			var str = "";
			var sel = $("select[id=room_building_select]");
	...
	function doEdit(){
		...
		var res = callService("/Building/building/getRoomById?bid="+InfoId);
...
//		}
	...
	function doAddEdit(){
		if (flag == "edit") {
			...
			$("form").attr("action","/Building/building/updateRoom");
			doEdit();
		} else if (flag == "add"){
			doAdd();
		}
	...
	form.on('submit(formDemo)', function(data){
		...
		$("#room_detail").val(layedit.getContent(layeditInstance));
