...
layui.use(['layedit','form', 'element','laydate'], function(){
	...
	var layeditInstance = layedit.build('permission_detail'); //建立编辑器
	/****************** 函数实现代码区域 start ******************/
	/**
	 * 
	 */
	...
	function doEdit(){
		...
		var res = callService("/Building/admin/getPermissionById?bid="+InfoId);
...
//		}
	...
	function doAddEdit(){
		if (flag == "edit") {
			...
			$("form").attr("action","/Building/admin/updatePermission");
			doEdit();
		} else if (flag == "add"){
			doAdd();
		}
	...
	form.on('submit(formDemo)', function(data){
		...
		$("#permission_detail").val(layedit.getContent(layeditInstance));
