...
layui.use(['layedit','form', 'element','laydate'], function(){
	...
	var layeditInstance = layedit.build('role_detail'); //建立编辑器
	/****************** 函数实现代码区域 start ******************/
	/**
	 * 
	 */
	...
	function getRoleData(){
		$.ajax({
			url:"/Building/admin/allRole",
			dataType:"json",
			success: function(data){
				if (data && data.code == 0){
					renderRoleField(data.data);
				}
	...
	function doEdit(){
		...
		var res = callService("/Building/admin/getRoleById?bid="+InfoId);
...
//		}
	...
	function doAddEdit(){
		if (flag == "edit") {
			...
			$("form").attr("action","/Building/admin/updateRole");
			doEdit();
		} else if (flag == "add"){
			doAdd();
		}
	...
	form.on('submit(formDemo)', function(data){
		...
		$("#role_detail").val(layedit.getContent(layeditInstance));
